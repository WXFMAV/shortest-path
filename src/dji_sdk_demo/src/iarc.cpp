...
double CIARC::TreeRobotValue(const IARC_ROBOT &robot)
{
      ...
      if(fabs(x0)<10.0 && fabs(y0)<10.0 && fabs(x1)<10.0 && fabs(y1)<10.0 && fabs(x2)<10.0 && fabs(y2)<10.0)
      {
            ...
            return ((500.0*(l0-(l1+l2+l3)/3.0)/l0)*TREE_SEARCH_GAMA_DISTANCE+(M_PI/2.0-fabs(fabs(theta)-M_PI/2.0))/M_PI*100.0*TREE_SEARCH_GAMA_THETA)*TREE_SEARCH_GAMA_INNER;//改变成为
            //return 0.0;
      }
      else
      {//出赛场，给出赛场一个评估函数
...
int CIARC::TaskPlan()
{
     ...
     {
          // if(GetNowTime()%PERIOD_TURN<15000 || GetNowTime()%PERIOD_TURN>2000)
           {
                 TaskListGenerate();
           }
           //SimulatorTask();
...
int CIARC::PathPlan()
{
      ...
      {
            fprintf(fp_pathlist,"%d task seq:%d task type:%d(%s) wp_pos:%d pathlist size:%d\n",GetNowTime(),task.seq,task.type,str_task_type[task.type],wp_pos,path_list.size());
      }
      return 0;
}
...
int CIARC::MotionPlan()
{
     ...
     if(task_pos>=0 && task_pos<task_list.size())
     {
           if(task_list[task_pos].type==type_interact && GetNowTime()>=task_list[task_pos].start_time && GetNowTime()<task_list[task_pos].start_time+task_list[task_pos].cost_time)
           {
                 IARC_TASK *p=&task_list[task_pos];
...
//           double OptimalController(double a0,double k0,double tgo,double x1,double x2,double xt1,double xt2);
     ...
     else
     {
           ...
           fprintf(fp_controller,"%d, task_seq:%d error wp(pos: %d size:%d) mav(%.2lf %.2lf) u(%.2lf %.2lf) cmd(%d,%d)\n",
                                   GetNowTime(),task_seq,wp_pos,path_list.size(),
                                  mav.x,mav.y,command.mav_vx,command.mav_vy, command.robot_turn_id, command.robot_turn_kind);
     }
