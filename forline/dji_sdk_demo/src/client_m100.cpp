...
ros::Time time_start;
static int use_m100 = 1;
static DJIDrone* drone = NULL;
int DroneActivate(DJIDrone* drone);
int DroneFeedback(DJIDrone* drone, CIARC * theIARC);
int DroneControl(DJIDrone* drone, CIARC * theIARC);
...
int DroneFeedback(DJIDrone* drone, CIARC * theIARC)
{
	theIARC->mav.x=drone->local_position.x;
	theIARC->mav.y=drone->local_position.y;
	theIARC->mav.z=drone->local_position.z;
	theIARC->mav.vx=drone->velocity.vx;
	theIARC->mav.vy=drone->velocity.vy;		        
	theIARC->mav.vz=drone->velocity.vz;		        	 	 		        
	theIARC->mav.ax=drone->acceleration.ax;
	theIARC->mav.ay=drone->acceleration.ay;
 	theIARC->mav.az=drone->acceleration.az; 	  	 
 	theIARC->mav.orientation.w=drone->attitude_quaternion.q0;
	theIARC->mav.orientation.x=drone->attitude_quaternion.q1;
	theIARC->mav.orientation.y=drone->attitude_quaternion.q2;
	theIARC->mav.orientation.z=drone->attitude_quaternion.q3;	 	
	printf("r(%.2lf %.2lf %.2lf) v(%.2lf %.2lf %.2lf) a(%.2lf %.2lf %.2lf) az(%.2lf %.2lf %.2lf %.2lf)\n",
	theIARC->mav.x,
	theIARC->mav.y,
	theIARC->mav.z,
	theIARC->mav.vx,
	...
	theIARC->mav.ay,
 	theIARC->mav.az,
 	theIARC->mav.orientation.w,
	theIARC->mav.orientation.x,
	theIARC->mav.orientation.y,
	theIARC->mav.orientation.z);	
	return 0;
}
int DroneControl(DJIDrone* drone, CIARC * theIARC)
{
	double cmd_vx,cmd_vy,cmd_vz,cmd_vyawdegree;
	cmd_vx=theIARC->command.mav_vx;	
	cmd_vy=theIARC->command.mav_vy;
	cmd_vz=theIARC->command.mav_vz;
	cmd_vyawdegree=0.0;	
//	if( drone->rc_channels.gear)			check if in P mode
...
//	printf("mode:%.2lf\n",drone->rc_channels.mode);-8000(P)-0(A)-8000(F)
    if( drone_permission == 1 )
    {
	    printf("vxyzyaw(%.2lf %.2lf %.2lf %.2lf)\n",cmd_vx,cmd_vy,cmd_vz,cmd_vyawdegree);	
...
int DroneActivate(DJIDrone* drone)
{
	if(drone!=NULL)
	{
		...
		for(int i=0;i<10;i++)
		{
			drone->activate();
			usleep(10000);
			printf("drone activation times=%d\n",drone->activation);
			if(!drone->activation)
			{
			    ROS_INFO("drone activation success ok !!");
				return 1;
			}
		}
		printf("drone activation failed!\n");
	}
	return 0;
}
uint32_t TimePassedMs(ros::Time start,ros::Time now)
