...
//	cout<<"tracing path"<<endl;
	...
	if(cmp_weight(dist[u], weight_max) != 0) found = true;

	while(cmp_weight(dist[u] , weight_max) != 0){

		if(dist[u] < value_nosolution_jump){
			path.push_back(u);
			if(cmp_weight(total_len, 0.0) == 0){
					total_len = dist[u];
			}
		}
		else{
			found = false;
			if(cmp_weight(total_len, 0.0) == 0){
					total_len = dist[u] - value_nosolution_jump;
			}
		}
	//	cout<<u<<" "<<dist[u]<<endl;
		if ( u == src) break;
...
bool dijkstra_foundmin(struct Graph* graph, int src, int tgt, double value_nosolution_jump, std::vector<int> &path, double &total_len) {
	...
	while (1) {
		// 取得堆顶节点，即最小距离的顶点
		int u=-1;
		weight_type dismin=weight_max;
		for(int v=0; v < V; ++v)
			if(open[v] && cmp_weight(dist[v],dismin)==-1){
				dismin=dist[v];
				u=v;
			}
		if(cmp_weight(dismin,weight_max)==0){
			break;
		}
		//cout << u << " "<<dist[u]<<endl;
		//if ( u == tgt ) break; //找到目标点，不必再搜索。。
		...
		while (pCrawl != NULL) {
			int v = pCrawl->dest;
			// 松弛操作，更新距离
			//if (open[v] && cmp_weight(dist[u], weight_max) != 0){
				weight_type newdis = dist[v];
				int cmp = cmpEdge(u, v, pCrawl->weight, dist[u], dist[v], newdis);
			//	cout << "exp :u-v w cmp "<<u<< ' '<<v<< ' '<<pCrawl->weight<<' '<<cmp<<endl;
				if(cmp == -1){
